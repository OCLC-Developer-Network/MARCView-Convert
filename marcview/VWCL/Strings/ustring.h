...
void f(const OLECHAR *pwsz, HWND hwnd)
{
	...
	Overloads to correct copy/conversion routine
	ustrcpy(sz, pwsz);

	_U temporarily dups buffer if needed
	SetWindowText(hwnd, _U(pwsz));
    GetWindowText(hwnd, sz, MAX_PATH);

	_UNCC needed because api non-const-correct
	CLSIDFromString(_UNCC(sz), &guid);
}
Copyright 1997, Don Box.*/
