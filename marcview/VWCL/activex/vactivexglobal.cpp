...
VBOOL VIsValidStructuredStorageName(VSTRING_CONST pszName)
{
	...
	{
		/* Return index of first bad char.*/
...
VBOOL VCoStringFromString(VSTRING* ppszCoString, VSTRING_CONST pszString)
{
	VASSERT(ppszCoString && pszString)
	...
	*ppszCoString = NULL;
	...
	if ( pszString )
	{
		...
		*ppszCoString =	(VSTRING)CoTaskMemAlloc(VSTRLEN(pszString) + 1);
